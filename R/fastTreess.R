#' Lists all available methods for computing the ESS of trees/multivariate objects for large samples of trees.
#'
#' @param recomended If TRUE, just returns list of the best-performing methods.
#' @export
#' @examples
#' getESSMethods()
getFastESSMethods <- function(recommended=FALSE) {
  all_methods <- c(
    # ESS = n (not generally useful, but helpful if testing new ESS functions, and it's certainly fast!)
    "fixedN",
...
#' treess(chains,dist)
fastTreess <- function(x,dist.fn,methods=getESSMethods(recommended=TRUE),alpha=0.05,nsim=1000,min.nsamples=5,max.approximateESS.timelag=100) {
