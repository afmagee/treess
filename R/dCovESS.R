...
#' @param use.median: Whether to summarize the jump distance distribution at each time lag by the median (if TRUE), otherwise the mean is used.
#' @keywords internal
dCorESS <- function(dmat,trees=NA,min.nsamples=5,nsim=1000,alpha=0.05,bootstrap=TRUE,interpolate=TRUE) {
  # recover()
  ...
  null_dist <- sapply(1:nsim,function(i){
    ...
    energy::dcor(permute[1:(n-1),1:(n-1)],permute[2:n,2:n],index=1.0)
  })
  threshold <- quantile(null_dist,probs=1 - alpha)

  thin <- n-min.nsamples+1
  dc <- rep(NA,n-min.nsamples+1)
  dc[1] <- energy::dcor(dmat[1:(n-1),1:(n-1)],dmat[2:n,2:n],index=1.0)
  for (i in 2:(n-min.nsamples+1)) {
    dc[i] <- min(dc[i-1],energy::dcor(dmat[-c(1:i),-c(1:i)],dmat[-c((n-i+1):n),-c((n-i+1):n)],index=1.0))
    if ( dc[i] < threshold ) {
      thin <- i
      break
    }
